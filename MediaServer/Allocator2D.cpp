...
CAllocator::CAllocator(HRESULT& hr, HWND wnd, int iIndex, IMediaSeeking *pMS, IDirect3D9* d3d, IDirect3DDevice9* d3dd)
: m_refCount(1)
, m_D3D(d3d)
, m_D3DDev(d3dd)
, m_window( wnd )
            ...
            /* [out][in] */ DWORD *lpNumBuffers)
{
    ...
    if(FAILED(hr) && !(lpAllocInfo->dwFlags & VMR9AllocFlag_3DRenderTarget))
    {
		...
		FAIL_RET( m_lpIVMRSurfAllocNotify[m_surfaces.size() - 1]->
			AllocateSurfaceHelper(lpAllocInfo, lpNumBuffers, & m_surfaces.at(m_surfaces.size() - 1) ) );
    }

	hr = m_scene.Init(m_D3DDev, g_pMediaSeeking, lpAllocInfo->dwWidth, lpAllocInfo->dwHeight);
        ...
        /* [in] */ IVMRSurfaceAllocatorNotify9 *lpIVMRSurfAllocNotify)
{
    ...
    FAIL_RET( m_lpIVMRSurfAllocNotify[m_surfaces.size()]->SetD3DDevice( m_D3DDev, hMonitor ) );
    ...
    /* [in] */ VMR9PresentationInfo *lpPresInfo)
{
    ...
    if( hr == D3DERR_DEVICELOST)
    {
        if (m_D3DDev->TestCooperativeLevel() == D3DERR_DEVICENOTRESET) 
        {
            ...
            FAIL_RET( m_lpIVMRSurfAllocNotify[0]->ChangeD3DDevice( m_D3DDev, hMonitor ) );
			FAIL_RET( m_lpIVMRSurfAllocNotify[1]->ChangeD3DDevice( m_D3DDev, hMonitor ) );
