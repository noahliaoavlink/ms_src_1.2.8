...
bool TCServerDll::LoadLib()
{
	...
	if (m_hInst)
	{
		...
		_Net_TCS_IsWorking = (_Net_TCS_IsWorking_Proc)GetProcAddress(m_hInst, "_Net_TCS_IsWorking");
...
bool TCServerDll::IsWorking()
{
	if (m_pTCSObj)
		return _Net_TCS_IsWorking(m_pTCSObj);
	return false;
}

void TCServerDll::UpdateCurFrameTime(unsigned long ulTime)
{
	if (m_pTCSObj)
		_Net_TCS_UpdateCurFrameTime(m_pTCSObj, ulTime);
}

int TCServerDll::GetTotalOfClients()
{
	if (m_pTCSObj)
