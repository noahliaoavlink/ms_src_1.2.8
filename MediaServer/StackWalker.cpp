...
void StackWalker::OpenFile(char* szFileName)
{
	fp = fopen(szFileName,"w+");
}

void StackWalker::CloseFile()
{
	if(fp)
		fclose(fp);
}
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  ...
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    if (entry.lineFileName[0] == 0)
    {
	  ...
	  _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%s+%x\n",entry.moduleName, (LPVOID)(entry.offset & 0xffff));
	  OnOutput(buffer);
