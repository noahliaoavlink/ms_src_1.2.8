...
bool  _CPUSupportCPUID()
{
	...
	try
	{
		...
		return (CPUIDInfOld != CPUIDInfNew);  // EFLAGS 第 21位 可以改?
	}
	catch (...)
	{
		return false;
...
bool  _CPUSupportMMX()  //判?CPU是否支持MMX指令
{
	if (!_CPUSupportCPUID())
		return false;
...
void MixAudioFormat(Uint8 * dst, const Uint8 * src, int format,Uint32 len, int volume)
{
	...
	switch (format)
	{
		...
		case AUDIO_S16LSB:
			...
			/*
				if(_CPUSupportMMX())
					SDL_MixAudio_MMX_S16_VC((char*)dst, (char*)src, len,volume);
				else */
				{
					Sint16 src1, src2;
					int dst_sample;
