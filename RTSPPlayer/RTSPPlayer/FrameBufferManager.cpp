...
void FrameBufferManager::AddToQueueBuffer(RtspFrameInfo* pInfo)
{
#if (_ENABLE_DUAL_BUFFER == 1)
	...
	int iHeadLen = sizeof(RtspFrameInfo) - sizeof(unsigned char*);
...
RtspFrameInfo* FrameBufferManager::GetNext()
{
#if (_ENABLE_DUAL_BUFFER == 1)
	if(!m_pFrameQBuffer[m_iCurQBIndex])
#else
	if(!m_pFrameQBuffer)
#endif
		return 0;
	...
	if(m_pFrameQBuffer->GetTotal() > 0)
	{
		...
		if(iLen > 0)
		{
			m_pStreamFrame = (RtspFrameInfo*)m_pTempFrameBuffer2;
			//int iHeadLen = sizeof(StreamFrame) - sizeof(unsigned char*);
			int iHeadLen = sizeof(RtspFrameInfo);

			m_pStreamFrame->pBuffer = (m_pTempFrameBuffer2 + _HEADER_LEN);
